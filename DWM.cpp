...
class DynamicWaveletMatrix{
  ...
  struct SeqNode{SeqNode*l,*r;int pr,sz;T val,sum,mn,mx;SeqNode(T v):l(nullptr),r(nullptr),pr(rand()),sz(1),val(v),sum(v),mn(v),mx(v){}};
  struct BitNode{BitNode*l,*r;int pr,sz,ones;bool bit;BitNode(bool b):l(nullptr),r(nullptr),pr(rand()),sz(1),ones(b),bit(b){}};
  SeqNode* seqRoot; array<BitNode*,B> bitRoots;
  ...
  static SeqNode* merge(SeqNode* a,SeqNode* b){
    if(!a||!b) return a?a:b;
    if(a->pr<b->pr){ a->r=merge(a->r,b); return pull(a); }
    else{ b->l=merge(a,b->l); return pull(b); }
  }
  static int sz(BitNode* x){ return x?x->sz:0; }
  static int ones(BitNode* x){ return x?x->ones:0; }
  static BitNode* pull(BitNode* x){
    x->sz=1+sz(x->l)+sz(x->r);
    x->ones=x->bit+ones(x->l)+ones(x->r);
    return x;
  }
  ...
  static int rank1(BitNode* t,int k){
    if(!t||k<=0) return 0;
  ...
  static bool at(BitNode* t,int k){
    while(t){int ls=sz(t->l);
      if(k<ls) t=t->l;
...
public:
  ...
  void insert(int pos,T v){
    SeqNode *a,*b; split(seqRoot,pos,a,b);
    seqRoot=merge(a,new SeqNode(v)); seqRoot=merge(seqRoot,b);
    U uv=v;
    for(int d=0;d<B;d++){
      auto &r=bitRoots[d];
  ...
  void erase(int pos){
    SeqNode *a,*m,*b; split(seqRoot,pos,a,m);
    ...
    for(int d=0;d<B;d++){
      ...
      split(R,1,M,R); delete M; r=merge(L,R); pos=npos;
    }
  }
  void updateValue(int p,T v){ erase(p); insert(p,v); }
  T rangeSum(int l,int r){
    SeqNode *a,*m,*b; split(seqRoot,l,a,m);
    split(m,r-l,m,b);
    T res=sum(m); seqRoot=merge(a,merge(m,b)); return res;
  }
  ...
  int rangeLessEqual(int l,int r,T v){
    U kv=v; if(kv==numeric_limits<U>::max()) return r-l; kv++;
    int cnt=0;
    for(int d=0; d<B; ++d){
